...
output_dir = "spray_shape_test_results"
os.makedirs(output_dir, exist_ok=True)

bridge = DrawingCanvasBridge()
bridge.start_canvas_interface()

def draw_spray_stroke_js(bridge, x_coords, y_coords, step_length, step_duration):
    js_code = f'''
        ...
        for (let i = 0; i < x_coords.length - 1; i++) {{
            ...
            for (let s = 0; s <= steps_per_segment; s++) {{
                ...
                if ((window.mouseX !== window.pmouseX) || (window.mouseY !== window.pmouseY)) {{
                    if (typeof window['sprayPaint'] === 'function') {{
                        window['sprayPaint']();
                    }}
                }}
...
try:
    for i in range(10):
        ...
        for shape, x_offset in zip(shapes, x_offsets):
            ...
            draw_spray_stroke_js(bridge, x, y, step_length, step_duration)
        time.sleep(1.2)
        filename = f"{output_dir}/spray_allshapes_len{step_length}_dur{step_duration}.png"
