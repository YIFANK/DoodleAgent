...
class LLMArtist:
    ...
    async def analyze_canvas_with_claude(self, canvas_data, current_settings, ai_position, websocket):
        """Send canvas image to Claude for creative analysis and next action decision"""
        try:
            # Check if we should stop before starting
            if self.should_stop:
                print("🛑 Stop requested, cancelling analysis")
                return None
            ...
            # Check if we should stop after image processing
            if self.should_stop:
                print("🛑 Stop requested during image processing")
                self.is_processing = False
                return None
    ...
    Respond with a JSON action that either EXPERIMENTS with new parameter combinations or BUILDS upon previous discoveries."""
            ...
            # Check if we should stop before sending to Claude
            if self.should_stop:
                print("🛑 Stop requested before Claude API call")
                self.is_processing = False
                return None
            ...
            # Update status to show we're calling Claude
            await websocket.send(json.dumps({
                "type": "thinking",
                "show": True,
                "message": "🤖 Asking Claude for creative analysis..."
            }))
            ...
            # Check if we should stop after Claude responds
            if self.should_stop:
                print("🛑 Stop requested after Claude response, finishing current thought")
                self.is_processing = False
                # Send one final message that we're stopping
                await websocket.send(json.dumps({
                    "type": "thinking",
                    "show": False
                }))
                await websocket.send(json.dumps({
                    "type": "ai_stopped",
                    "message": "🛑 LLM Artist stopped gracefully"
                }))
                return None
    ...
    def validate_action(self, action, canvas_data):
        ...
        if 'brush_size' in action:
            action['brush_size'] = max(5, min(30, action['brush_size']))
        ...
        if 'turbulence' in action:
             action['turbulence'] = max(0, min(6, action['turbulence']))
            ...
            async for message in websocket:
                try:
                    ...
                    if data['type'] == 'canvas_state':
                        # Only process if we're not stopping
                        if not self.should_stop:
                            ...
                            print("🛑 Canvas state ignored due to stop request")

                    elif data['type'] == 'ai_control':
                        if data['command'] == 'start':
                            print("🎨 AI started by frontend")
                            self.should_stop = False
                            await websocket.send(json.dumps({
                                "type": "ai_started",
                                "message": "🎨 LLM Artist is now active"
                            }))
                        elif data['command'] == 'stop':
                            print("🛑 Stop requested by frontend")
                            self.should_stop = True
                            if self.is_processing:
                                print("⏳ Will stop after current thought completes...")
                                await websocket.send(json.dumps({
                                    "type": "thinking",
                                    "show": True,
                                    "message": "🛑 Finishing current thought, then stopping..."
                                }))
                            else:
                                await websocket.send(json.dumps({
                                    "type": "ai_stopped",
                                    "message": "🛑 LLM Artist stopped"
                                }))
